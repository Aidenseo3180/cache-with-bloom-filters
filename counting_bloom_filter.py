...
class BloomFilter:
    ...
    def insert(self, x):
        for i in range(self.k):
            ...
            self.t[i][idx] += 1         #add to the bloom filter
...
def findCountingBloomFilter(k, m):
    ...
    with open(filename) as f_in:
        ...
        for row in reader:
            ...
            if row[0] in cache_list:
                continue
            #if not in cache list but we're visiting the same URL (now we know that it should be cached), then insert to cache list
            if bf.check(row[0]):
                #since we're adding to the cache list, remove it from hash map (count down)
                bf.remove(row[0])
    ...
    #print("Expected FPR: {0:.5f}".format(pow(1 - (1 - pow(1/m, len(valid_cache_set))), k)))
...
if __name__ == "__main__":
    ...
    for i in range (1, 11):
        ...
        for j in range(1000, 8001, 1000):
            print(i, j)
            findCountingBloomFilter(i, j) # 10 * 8,000 = 80,000 bits = 10 KB
